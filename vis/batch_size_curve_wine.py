"""
Visualiza the experiment results about effect of batch size on the training of QNN (Figure 6 in the paper)
"""
import numpy as np
import math
...
fig, ax = plt.subplots(1, 2, sharex='col', sharey='row', figsize=(11.08, 5.0))
lines, lines_label = [], []
x = range(0, 100, 10)
for k, net in enumerate(['qnn', 'qnn_embedding']):
    ...
    for j, config in enumerate(results):
        for mode in ['train', 'test']:
            ...
            line = ax[k].plot(x, result[::10], color=colors[mode][j], marker=markers[j])[0]
...
plt.tight_layout()
# fig.legend(lines[:len(lines)//2], labels=lines_label[:len(lines_label)//2], loc='center right', borderaxespad=0.2, fontsize=20, labelspacing=0.5)
# plt.subplots_adjust(right=0.82)
fig.legend(handles=lines[:len(lines)//2], labels=lines_label[:len(lines_label)//2], loc='upper left', borderaxespad=0.2, mode="expand", fontsize=20, ncol=5)
plt.subplots_adjust(top=0.75)
plt.savefig('test/net_bSGD_wine.pdf', dpi=600, format='pdf')
plt.show()
plt.clf()
...
for i, net in enumerate(['qnn', 'qnn_embedding']):
    ...
    for j, config in enumerate(results):
        result_seed = []
        for k in range(10):
            result_path = os.path.join(root, net, str(k), '_'.join(['acc', 'train', net, config]))
            if not os.path.exists(result_path):
                continue
            result = np.load(result_path)
            if not isinstance(result.tolist(), list) or len(result) < 2:
                continue
            result_seed.append(result[-1])
        data.append(np.mean(result_seed))
        std.append(np.std(result_seed))
    plt.plot(xticks, data, label=title[i], color=colors['train'][i], marker=markers[i])
    plt.fill_between(xticks, np.array(data)+np.array(std), np.array(data)-np.array(std), color=colors['train'][i], alpha=0.1)
    plt.tick_params(labelsize=20)
# plt.legend(loc='best', fontsize=20)
plt.legend(loc='lower left', ncol=2, bbox_to_anchor=(0., 1.02, 1., .102), fontsize=20, borderaxespad=1.0, mode="expand")
plt.subplots_adjust(top=0.9)
plt.xlabel('Batch size', fontsize=20)
plt.ylabel('Accuracy', fontsize=20)
plt.tight_layout()
plt.savefig('figure/acc_bs_wine.pdf', dpi=600, format='pdf')
plt.show()
plt.clf()